...
impl ShodhConnector {
    pub(crate) fn new(config: &ShodhEnrichmentConfig) -> anyhow::Result<Self> {
        ...
        if endpoint.is_empty() {
            anyhow::bail!(
                "memory enricher 'shodh' requires `endpoint` in \
        ...
        if endpoint.starts_with(r"\\.\pipe\") {
            anyhow::bail!(
                "Shodh IPC endpoint '{endpoint}' is a Windows named pipe, but this platform is {}; \
                 configure a Unix-domain socket path",
                std::env::consts::OS
            );
        }
        let api_key = config
    ...
    async fn request<T: Serialize>(
        ...
        let Some(response) = response.strip_suffix(b"\n") else {
        ...
        // An empty id reports a server-side protocol error that happened before
        // the request id could be recovered. Preserve its actual status and body.
        ...
        if !(200..300).contains(&response.status) {
            let detail = self.format_response_body(&response.body);
            anyhow::bail!(
    ...
    async fn exchange(&self, request: &[u8]) -> anyhow::Result<Vec<u8>> {
        let stream = open_local_stream(&self.endpoint).await?;
    ...
    fn format_response_body(&self, body: &Value) -> String {
        let body = match self.redacted_response_value(body) {
    ...
    fn redacted_response_value(&self, value: &Value) -> Value {
        match value {
            Value::String(value) => Value::String(self.redact(value)),
            Value::Array(values) => Value::Array(
                values
                    .iter()
                    .map(|value| self.redacted_response_value(value))
                    .collect(),
            ),
            Value::Object(values) => Value::Object(
                values
                    .iter()
                    .map(|(key, value)| (self.redact(key), self.redacted_response_value(value)))
                    .collect(),
            ),
            value => value.clone(),
        }
...
mod tests {
    ...
    fn test_config(endpoint: &Path) -> ShodhEnrichmentConfig {
    ...
    #[test]
    fn constructor_requires_explicit_endpoint() {
    ...
    fn constructor_requires_api_key() {
        let config = ShodhEnrichmentConfig {
            endpoint: "shodh-test.sock".to_string(),
            ..ShodhEnrichmentConfig::default()
        };
    ...
    #[cfg(not(windows))]
    #[test]
    fn constructor_rejects_windows_pipe_on_unix() {
        let config = ShodhEnrichmentConfig {
            endpoint: r"\\.\pipe\shodh-memory".to_string(),
            api_key: Some("test-api-key".to_string()),
            ..ShodhEnrichmentConfig::default()
        };
        let error = match ShodhConnector::new(&config) {
            Ok(_) => panic!("a Windows named pipe must not be treated as a Unix socket"),
            Err(error) => error,
        };
        assert!(error.to_string().contains("is a Windows named pipe"));
    }
    ...
    async fn agent_scoped_store_and_recall_use_authenticated_local_ipc() {
        ...
        assert!(
            requests
                .iter()
                .all(|request| request.as_object().unwrap().len() == 6)
        );
    ...
    #[tokio::test]
    async fn empty_response_id_surfaces_the_protocol_error() {
        let temp = TempDir::new().unwrap();
        let (socket_path, _, server) = spawn_fake_server(
            &temp,
            1,
        ...
        assert!(error.contains("INVALID_ENVELOPE"));
        assert!(!error.contains("id did not match"));
        finish_fake_server(server).await;
    }
    ...
    async fn non_success_response_does_not_expose_auth_secret() {
        let temp = TempDir::new().unwrap();
        let api_key = r#"test-"api\key"#;
        let leaked_api_key = api_key.to_string();
        let (socket_path, _, server) = spawn_fake_server(
            &temp,
            1,
            responder(move |_| {
                FakeResponse::error(401, &format!("authentication failed for {leaked_api_key}"))
            }),
        );
        let mut config = test_config(&socket_path);
        ...
        assert!(!error.contains(r#"test-\"api\\key"#));
    ...
    async fn oversized_request_is_rejected_before_connecting() {
        let config = ShodhEnrichmentConfig {
            endpoint: "definitely-missing-shodh.sock".into(),
            api_key: Some("test-api-key".into()),
            ..ShodhEnrichmentConfig::default()
        };
        let connector = ShodhConnector::new(&config).unwrap();
