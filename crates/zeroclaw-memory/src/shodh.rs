use super::enriched::{
    ...
    EnrichmentPolicy, EnrichmentRecallRequest, EnrichmentStoreRequest, MemoryEnricher, RecallScope,
    RecallSupport, ResultKind,
};
...
use zeroclaw_config::schema::ShodhStorageConfig;
...
/// HTTP connector for a Shodh Memory sidecar.
...
pub type ShodhMemory = EnrichedMemory<ShodhConnector>;
...
impl ShodhConnector {
    fn new(config: &ShodhStorageConfig) -> anyhow::Result<Self> {
        ...
        let api_key = config
            ...
            .ok_or_else(|| {
                anyhow::Error::msg(
                    "memory backend 'shodh' requires `api_key` in [storage.shodh.<alias>]",
                )
            })?
            .to_string();
...
impl EnrichedMemory<ShodhConnector> {
    /// Construct local-authoritative Shodh enrichment from typed storage config.
    pub fn from_config(
        alias: &str,
        local: SqliteMemory,
        config: &ShodhStorageConfig,
    ) -> anyhow::Result<Self> {
        if config.local_hit_threshold == 0 {
            anyhow::bail!("Shodh local_hit_threshold must be greater than zero");
        }
        let connector = ShodhConnector::new(config)?;
        Ok(Self::from_parts(
            alias,
            local,
            connector,
        ...
        match request.allowed_agent_ids {
            Some(agent_ids) => {
                ...
                for agent_id in agent_ids.iter().copied() {
                    match self.recall_user(agent_id, Some(agent_id), request).await {
                        Ok(agent_results) => {
...
mod tests {
    ...
    fn test_config(server: &MockServer) -> ShodhStorageConfig {
        ShodhStorageConfig {
            base_url: server.uri(),
    ...
    fn constructor_requires_api_key() {
        let error = match ShodhConnector::new(&ShodhStorageConfig::default()) {
    ...
    async fn agent_scoped_store_and_recall_use_authenticated_http_api() {
        ...
        let memory = ShodhMemory::from_config("shodh", local, &test_config(&server)).unwrap();
