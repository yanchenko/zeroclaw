...
pub type LucidMemory = EnrichedMemory<LucidConnector>;
...
impl EnrichedMemory<LucidConnector> {
    ...
    /// Construct the existing local-first Lucid backend with default policy.
    pub fn new(alias: &str, workspace_dir: &Path, local: SqliteMemory) -> Self {
        Self::from_parts(
            alias,
            local,
            LucidConnector::new(workspace_dir),
...
"#
    ...
    fn test_memory(workspace: &Path, command: String) -> LucidMemory {
