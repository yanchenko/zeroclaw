...
| Conversation memory | `zeroclaw-memory` backend selected per agent | SQLite/Postgres/Lucid/Shodh/Qdrant/Markdown backend locations; SQLite shared store lives under `data/memory/` | `Arc<dyn Memory>` wrapped in agent-scoping adapters | Backend choice is locked once an agent has written data; same-backend cross-agent recall is opt-in | Memory rows are agent-scoped. Lucid and Shodh keep SQLite authoritative while treating their external integration as best-effort enrichment. Do not replace memory ownership with copied prompt/session caches. |
...
External memory services are outside the install-root backup. For the Shodh
backend, `data/memory/` remains the authoritative ZeroClaw copy; back up Shodh
separately only when its derived semantic index must also be preserved.
