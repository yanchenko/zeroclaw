...
- **SQLite / Postgres / Lucid / Shodh**: shared install-wide store. The `agents` table maps alias → UUID, and the `memories` table carries `agent_id` referencing that UUID. The factory wraps the inner backend in `AgentScopedMemory`, which stamps the bound agent's UUID on every store via `store_with_agent` and filters every recall via `recall_for_agents` with the resolved allowlist. Lucid and Shodh use SQLite as the canonical local store. Shodh also uses each stable UUID as an isolated remote `user_id`, while Lucid does not support scoped external recall and therefore stays local-only on agent-scoped reads.
